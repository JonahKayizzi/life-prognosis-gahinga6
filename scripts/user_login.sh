...
if [ -n "$user_record" ]; then
    ...
    echo "$user_id $user_type $user_email $user_code $user_first_name $user_last_name $user_dob $user_hiv_status $user_country $user_diagnosis_date $user_is_on_art $user_art_start_date" > "$PWD/session-storage.txt"
